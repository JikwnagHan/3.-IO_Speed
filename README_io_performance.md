# IO 성능 시험 스크립트 안내 (io_performance_v_0.8.ps1)

## 개요
`io_performance_v_0.8.ps1`은 일반 영역과 보안 영역의 디스크 쓰기/읽기 속도를 비교하는 PowerShell 자동화 스크립트입니다. 동일한 드라이브를 입력하더라도 `Normal_Zone`, `Secure_Zone` 폴더를 별도로 만들어 충돌 없이 측정하며, 샘플 데이터를 여러 번 저장 및 읽어서 속도를 측정한 뒤 CSV, XLSX, DOCX 보고서를 생성합니다.
...
| `보안영역 디스크 드라이브 (예: E:\ 또는 D:\)` | 보안 구역으로 사용할 드라이브 문자를 입력합니다. 필요하면 일반 영역과 동일한 드라이브를 입력해도 됩니다. |
| `샘플 데이터 위치 (예: D:\Dataset, 기존 파일은 삭제하여 10개 샘플을 생성합니다.)` | 샘플 파일을 만들 폴더 경로를 입력합니다. 폴더가 없으면 만들어지고, 기존 파일은 제거됩니다. |
| `결과 데이터 저장 위치 (예: D:\logs)` | 결과 파일을 저장할 폴더를 입력합니다. 폴더가 없으면 자동으로 생성됩니다. |

## 스크립트가 수행하는 작업
1. 입력한 드라이브 문자를 정리해 `D:\` 형태로 변환합니다.
2. (동일 드라이브 입력 포함) 각 드라이브에 `Normal_Zone`, `Secure_Zone` 폴더를 만들고 기존 파일을 모두 비웁니다.
