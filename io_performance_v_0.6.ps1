...
# 드라이브 문자를 유연하게 입력할 수 있으며, Resolve-DriveRoot가 자동으로 표준화합니다.
...
$analysisParagraphs = @(
    # 워드 보고서에 들어갈 상세 해설 문단입니다.
    # 성능 수치뿐 아니라 이후 점검 시 참고할 수 있는 권장 조치도 포함했습니다.
    '',
    '평가 결과 요약',
...
$paragraphs = @(
    # 보고서 머리말과 기본 정보를 담은 문단입니다.
    # 실행 시점, 사용된 경로, 평균 측정값 등을 정리하여 감사 용도로도 활용할 수 있습니다.
    '입출력 속도 성능평가 자동화 보고서',
    "생성 일시: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",
    "Run ID: $runId",
    "난수 시드: $seed",
    "측정 반복 수: $iterations",
    "일반 영역: $normalZoneRoot",
    "보안 영역: $secureZoneRoot",
