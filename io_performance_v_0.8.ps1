...
# 드라이브 문자는 유연하게 입력할 수 있으며, 같은 드라이브를 입력해도 됩니다.
# Resolve-DriveRoot가 자동으로 표준화해 Normal/Secure 작업 폴더를 각각 만듭니다.
...
# 두 경로가 같은 드라이브를 가리키더라도 서로 다른 하위 폴더(Normal_Zone/Secure_Zone)를 사용하므로 충돌하지 않습니다.
